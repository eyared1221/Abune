...
export function ChildBottomNav({ active }: { active: ChildRoute }) {
  ...
  return <nav className="fixed inset-x-0 bottom-0 z-10 border-t border-[#29436c] bg-[radial-gradient(circle_at_10%_0,rgba(74,109,164,.35),transparent_35%),linear-gradient(115deg,#061c42,#0c2c5d)] px-3 pb-[calc(12px+env(safe-area-inset-bottom))] pt-3 text-white md:px-8 xl:hidden"><div className="mx-auto grid max-w-[700px] grid-cols-4">{items.map(({ key, label, href, icon: Icon }) => <button key={key} className={`relative flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-[19px] text-[11px] ${active === key ? "bg-white/10 text-[#f2b63a]" : "text-white"}`} onClick={() => router.push(href, { locale })} type="button"><Icon className="h-6 w-6" />{label}{active === key ? <span className="absolute bottom-0 h-1 w-1 rounded-full bg-[#f2b63a]" /> : null}</button>)}</div></nav>;
}
