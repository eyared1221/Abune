...
export function AppointmentCard({ appointment, onDelete, onEdit }: { appointment: ChildAppointment; onDelete?: () => void; onEdit?: () => void }) {
  ...
  return <article className="mobile-request-card"><span className="request-calendar"><CalendarClock /></span><div><b>{reason}</b><small>{date} at {appointment.requestedStartTime.slice(0, 5)}</small></div><em className={`request-status ${appointment.status.toLowerCase()}`}>{appointment.status.replace("_", " ")}</em>{pending && onEdit ? <button aria-label={`Edit ${reason} request`} onClick={onEdit} type="button"><Pencil /></button> : null}{pending && onDelete ? <button aria-label={`Delete ${reason} request`} className="delete" onClick={onDelete} type="button"><Trash2 /></button> : null}</article>;
}
