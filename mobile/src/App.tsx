...
function ChildPortal({ session, onLogout }: { session: { user: NonNullable<NonNullable<Session>["user"]> }; onLogout: () => Promise<void> }) {
  ...
  const active = location.pathname === "/child" || location.pathname.startsWith("/child/home/") ? "home" : location.pathname.startsWith("/child/appointments") ? "appointments" : location.pathname.startsWith("/child/messages") ? "messages" : location.pathname.startsWith("/child/more") ? "more" : "spiritual";
  const name = session.user.name?.trim() || "Child";
  return <main className="portal"><header><button className="wordmark" onClick={() => navigate("/child")} type="button">Abune</button><button aria-label="Open reminders" className="header-bell" onClick={() => navigate("/child/reminders")} type="button"><Bell /><i /></button></header><Routes><Route element={<ChildHomePage name={name} />} path="/child" /><Route element={<HomeFeaturePage />} path="/child/home/:feature" /><Route element={<AppointmentsPage />} path="/child/appointments" /><Route element={<NewAppointmentPage />} path="/child/appointments/new" /><Route element={<AppointmentDetailPage />} path="/child/appointments/:appointmentId" /><Route element={<MessagesPage />} path="/child/messages" /><Route element={<MorePage onLogout={onLogout} />} path="/child/more" /><Route element={<AboutPage />} path="/child/more/about" /><Route element={<CompleteProfilePage />} path="/child/more/profile" /><Route element={<MoreFeaturePage />} path="/child/more/:feature" /><Route element={<ProfilePage email={session.user.email} />} path="/child/profile" /><Route element={<RemindersPage />} path="/child/reminders" /><Route element={<SpiritualDatesPage />} path="/child/spiritual-dates" /><Route element={<TimelinePage />} path="/child/timeline" /><Route element={<Navigate replace to="/child" />} path="*" /></Routes><ChildNavigation active={active} /></main>;
}
